...
object TransformationOpScala {
  ...
  def distinctOp(sc: SparkContext): Unit = {
    val lp = Array((150001,"US"),(150002,"CN"),(150003,"CN"));
  ...
  def joinOp(sc: SparkContext): Unit = {
    val lp = Array((150001,"US"),(150002,"CN"));
    val ty = Array((150002,200),(150001,100));
    val dataRDD1 = sc.parallelize(lp)
    val dataRDD2 = sc.parallelize(ty)
  ...
  def sortByKeyOp(sc: SparkContext): Unit = {
    val ty = Array((150002,200),(150003,300),(150004,100));
  ...
  def reduceByKeyOp(sc: SparkContext): Unit = {
    val ty = Array((150001,"US"),(150002,"CN"),(150003,"CN"),(150004,"IN"))

    val dataRDD = sc.parallelize(ty)
    //由于这个需求只需要使用到大区信息，所以在mao操作的时候只保留大区信息即可
    dataRDD.map(tup=>(tup._2,8)).reduceByKey(_ + _).foreach(println(_))
  }

  def groupByKeyOp(sc: SparkContext): Unit = {
    val ar = Array((150001,"US"),(150002,"CN"),(150003,"CN"),(150004,"IN"))
    val dataRDD = sc.parallelize(ar)
  ...
  def groupByKeyOp2(sc: SparkContext): Unit = {
    val lp = Array((150001, "US","male"), (150002, "CN","female"), (150003, "CN","male"), (150004, "IN","female"))
    val dataRDD = sc.parallelize(lp)
    ...
    def flatMapOp(sc: SparkContext): Unit = {
      val ty = Array("good good study","day day up")
      val dataRDD = sc.parallelize(ty)
